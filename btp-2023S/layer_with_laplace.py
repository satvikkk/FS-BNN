...
class SpikeNLaplaceSlabLayer(nn.Module):
    """
    ...
    def __init__(self, input_dim, output_dim, rho_prior, device, rho0=-6., lambda0=0.99):
        # initialize layers
        super(SpikeNLaplaceSlabLayer, self).__init__()
    ...
    def forward(self, X, temp, phi_prior):
        ...
        epsilon_w = Laplace(0, 1).sample(self.w_mu.shape)
        epsilon_b = Laplace(0, 1).sample(self.b_mu.shape)
        ...
        kl_w = w_phi * (torch.log(w_phi) - torch.log(phi_prior)) + \
            ...
            w_phi * (sigma_prior * torch.exp(-torch.abs(self.w_mu) / sigma_w) / sigma_w + \
                     torch.abs(self.w_mu) / sigma_w + torch.log(sigma_w / sigma_prior) - 1)

        kl_b = b_phi * (torch.log(b_phi) - torch.log(phi_prior)) + \
            (1 - b_phi) * (torch.log(1 - b_phi) - torch.log(1 - phi_prior)) + \
            b_phi * (sigma_prior * torch.exp(-torch.abs(self.b_mu) / sigma_b) / sigma_b + \
                     torch.abs(self.b_mu) / sigma_b + torch.log(sigma_b / sigma_prior) - 1)
