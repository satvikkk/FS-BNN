...
class SpikeNSlabLayer(nn.Module):
    ...
    """
    # add flag to select gs or truncated gaussian
    ...
    def __init__(self, input_dim, output_dim, rho_prior, device, rho0=-6., lambda0=0.99):
        ...
        # if flag == True:
        #     self.w_theta = nn.Parameter(torch.Tensor(input_dim, output_dim).uniform_(-0.6, 0.6))
        #     self.w_sigma = nn.Parameter(torch.Tensor(input_dim, output_dim).uniform_(-0.6, 0.6))
        # else:
        #     print("flag is false1")
        ...
        # if flag:
        #     self.b_theta = nn.Parameter(torch.Tensor(input_dim, output_dim).uniform_(-0.6, 0.6))
        #     self.b_sigma = nn.Parameter(torch.Tensor(input_dim, output_dim).uniform_(-0.6, 0.6))
        # else:
        self.b_theta = nn.Parameter(logit(torch.Tensor(input_dim, output_dim).uniform_(lambda0, lambda0)))
    ...
    def forward(self, X, temp, phi_prior, flag = False): 
        ...
        #     self.gamma_w = stochastic_gate(self.w_theta, self.w_sigma, epsilon_wt, hard = True)
        #     self.gamma_b = stochastic_gate(self.b_theta, self.b_sigma, epsilon_bt, hard = True)
        # else:
        #     print("flag is false2")
        u_w = torch.rand(self.w_theta.shape) #rand() generates pseudorandom number
        u_b = torch.rand(self.b_theta.shape) #rand() generates pseudorandom number
        ...
        # if flag == True:
        #     w_phi = hard_sigmoid(self.w_theta + self.w_sigma * epsilon_wt)
        #     b_phi = hard_sigmoid(self.b_theta + self.b_sigma * epsilon_bt)
        # else:
        #     print("flag is false3")
