...
class SparseBNNClassification(nn.Module):
    ...
    def __init__(self, data_dim, device, target_dim=3, hidden_dim=5, sigma_noise=1.):
        ...
        self.l1 = SpikeNSlabLayer(data_dim, hidden_dim, self.rho_prior, self.device, flag=False)
        self.l1_relu = nn.ReLU()
        self.l2 = SpikeNSlabLayer(hidden_dim, hidden_dim, self.rho_prior, self.device, flag=False)
        self.l2_relu = nn.ReLU()
        self.l3 = SpikeNSlabLayer(hidden_dim, hidden_dim, self.rho_prior, self.device, flag=False)
        self.l3_relu = nn.ReLU()
        self.l4 = SpikeNSlabLayer(hidden_dim, target_dim, self.rho_prior, self.device, flag=False)
...
class FeatureSelectionBNNClassification(nn.Module):
    ...
    def sample_elbo(self, X, y, n_samples, temp, phi_prior, num_batches):
        ...
        # calculate negative elbo
        loss = kl_MC / num_batches + nll_MC
        print("Loss: {}, KL: {}, Log_likes: {}".format(loss, kl_MC / num_batches, nll_MC))
        return loss, torch.argmax(outputs, dim=-1).squeeze()