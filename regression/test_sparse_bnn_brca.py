...
from sparse_bnn import SparseBNN, FeatureSelectionBNN
...
dmatrix = pd.read_csv('dm_red.csv',index_col=0)
dframe = pd.read_csv('df_red.csv',index_col=0)
...
print(dframe.shape)
dframe.loc[dframe["subtype_PAM50.mRNA"].isna(), "subtype_PAM50.mRNA"] = "No Cancer" # TODO: REMOVE any hardcoding 
dframe["pam50"], mapping = pd.factorize(dframe["subtype_PAM50.mRNA"]) # TODO: REMOVE any hardcoding 
coded_categories = np.unique(dframe["pam50"]) # TODO: REMOVE any hardcoding 
mapping_dict = {} # TODO: not used, why do you need this? 
count = 0
for i in mapping:
	mapping_dict[i] = count
	count += 1
    
print("categories: ", coded_categories)
print("mapping", mapping)
print("mapping_dict", mapping_dict)
dframe[["subtype_PAM50.mRNA", "pam50"]].sample(10)  # TODO: REMOVE any hardcoding 

inputM = dmatrix.to_numpy() #X
print(dmatrix.head())
print(dmatrix.columns[3])
print(inputM.shape)
labels = dframe['pam50'].to_numpy()  # TODO: REMOVE any hardcoding 
print(labels)
labels2d = tf.keras.utils.to_categorical(labels) # TODO: REMOVE the dependency on tensorflow 
# USE this https://discuss.pytorch.org/t/is-there-something-like-keras-utils-to-categorical-in-pytorch/5960 
# labels = np.where(labels > 0, 1, labels) #y for 2 class problem

x_train, x_test, y_train, y_test = train_test_split(inputM, labels, test_size=(72/574)) # TODO: REMOVE hardcoding, e.g., test_size (it should be dynamic)
# X_train, X_valid, y_train, y_valid = train_test_split(X_train, y_train, test_size=(251/574))
...
for k in range(rep):
    ...
    # create sparse BNN
    net = SparseBNN(data_dim=data_dim, device=device).to(device)
    ...
    df.to_csv("brca_sparse_one1w", index = False)
