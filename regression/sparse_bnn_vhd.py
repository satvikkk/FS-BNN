...
class SparseBNN(nn.Module):
    ...
    def __init__(self, data_dim, device, target_dim=1, hidden_dim=[16,8,4], sigma_noise=1.):
        ...
        self.l1 = SpikeNSlabLayer(data_dim, hidden_dim[0], self.rho_prior, self.device)
        self.l1_relu = nn.ReLU()
        self.l2 = SpikeNSlabLayer(hidden_dim[0], hidden_dim[1], self.rho_prior, self.device)
        self.l2_relu = nn.ReLU()
        self.l3 = SpikeNSlabLayer(hidden_dim[1], hidden_dim[2], self.rho_prior, self.device)
        self.l3_relu = nn.ReLU()
        self.l4 = SpikeNSlabLayer(hidden_dim[2], target_dim, self.rho_prior, self.device)
