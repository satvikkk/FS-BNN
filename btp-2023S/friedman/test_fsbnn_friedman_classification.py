...
from tools import sigmoid, min_max
from sparse_bnn_classification import FeatureSelectionBNNClassification
...
curr_dir = "/Users/Akanksha Mishra/Documents/GitHub/sbnn/btp-2023S/friedman"
code = "fsbnn"
...
trainsets = []
for i in range(rep):
    x_train, y_train = generate_friedman_classification_dataset(data_size, data_dim)
...
test_size = 1000
x_test, y_test = generate_friedman_classification_dataset(test_size,data_dim)
...
for k in range(rep):
    ...
    # make_dot(net(x_train, temp, phi_prior), params=dict(list(net.named_parameters())), show_attrs=True, show_saved=True).render("sbnn_torchviz", format = "png")
    ...
    t_np = one1_w.detach().numpy()
    df = pd.DataFrame(t_np)
    df.to_csv(f"{curr_dir}/{code}/fsbnn_classification_GS_5k", index = False)
...
np.save(f"{curr_dir}/{code}/l1_wtheta", l1_wtheta)
