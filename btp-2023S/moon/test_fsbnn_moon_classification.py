...
from feature_selection_classification import FeatureSelectionBNNClassification
...
trainsets = []
for i in range(rep):
    x_train, y_train = create_twomoon_dataset(data_size, data_dim)
...
test_size = 1000
x_test, y_test = create_twomoon_dataset(test_size,data_dim)
...
for k in range(rep):
    ...
    for epoch in range(epochs): 
        ...
        one1_w = (min_max(net.l1.w_theta) > 0).float() 
    ...
    # sparsity level
    one1_w = (min_max(net.l1.w_theta) > 0).float()
    one1_b = (min_max(net.l1.b_theta) > 0).float()
    sparse_overall = ( torch.sum(one1_w) + torch.sum(one1_b) ) / total
    sparse_overalls.append(sparse_overall)
    sparse_overall2 = ( torch.sum(min_max(net.l1.w_theta)) + torch.sum(min_max(net.l1.b_theta)) ) / total
